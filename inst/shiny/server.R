...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_save_scenario, {
    ...
    if (is.null(res)) {
      showNotification("Run a simulation first.", type = "warning"); return()
    }
    if (nchar(name) == 0) {
      showNotification("Enter a scenario name.", type = "warning"); return()
    }
    current_list <- scenarios()
    if (length(current_list) >= 3L) {
      showNotification("Maximum 3 scenarios. Clear one first.", type = "warning")
      return()
    }
    current_list[[name]] <- res
    scenarios(current_list)
    showNotification(sprintf("Scenario '%s' saved.", name), type = "message")
  })

  observeEvent(input$btn_clear_scenarios, {
    scenarios(list())
    showNotification("All scenarios cleared.", type = "message")
  })

  output$scenarios_summary_table <- renderTable({
    sc <- scenarios()
    if (length(sc) == 0L) return(data.frame(Message = "No scenarios saved yet."))
    do.call(rbind, lapply(names(sc), function(nm) {
      k <- sc[[nm]]$kpis
      cfg <- sc[[nm]]$config
      data.frame(
        Scenario          = nm,
        Doctors           = cfg$n_doctors,
        Nurses            = cfg$n_nurses,
        `Arrival Rate`    = cfg$arrival_rate,
        `Duration (min)`  = cfg$sim_duration,
        `Patients Served` = k$n_patients,
        `Mean Wait (min)` = round(k$mean_wait, 1),
        `P95 Wait (min)`  = round(k$p95_wait,  1),
        `Throughput/hr`   = round(k$throughput_per_hour, 1),
  ...
  scenario_palette <- c("#3498db", "#e74c3c", "#2ecc71")

  output$compare_wait_plot <- renderPlot({
    sc <- scenarios()
    if (length(sc) == 0L) return(NULL)
    df <- do.call(rbind, lapply(names(sc), function(nm) {
      k <- sc[[nm]]$kpis$wait_by_urgency_df
      k$scenario <- nm
      k
    }))
    df$urgency_label <- factor(df$urgency_label,
                               levels = c("Critical", "Urgent", "Standard"))
    ggplot(df, aes(x = urgency_label, y = mean_wait, fill = scenario)) +
      geom_bar(stat = "identity", position = "dodge", alpha = 0.85) +
      scale_fill_manual(values = scenario_palette[seq_along(sc)]) +
      labs(x = "Urgency", y = "Mean Wait (min)", fill = "Scenario") +
      theme_minimal(base_size = 13)
  })

  output$compare_throughput_plot <- renderPlot({
    sc <- scenarios()
    if (length(sc) == 0L) return(NULL)
    df <- data.frame(
      scenario   = names(sc),
      throughput = vapply(sc, function(s) s$kpis$throughput_per_hour, double(1L))
    )
    ggplot(df, aes(x = scenario, y = throughput, fill = scenario)) +
      geom_bar(stat = "identity", alpha = 0.85) +
      scale_fill_manual(values = scenario_palette[seq_along(sc)]) +
      labs(x = NULL, y = "Patients / Hour", fill = "Scenario") +
      theme_minimal(base_size = 13) +
      theme(legend.position = "none")
  })

  output$compare_utilisation_plot <- renderPlot({
    sc <- scenarios()
    if (length(sc) == 0L) return(NULL)
    df <- do.call(rbind, lapply(names(sc), function(nm) {
      rl <- sc[[nm]]$resource_log
      rl$scenario <- nm
      rl
    }))
    ggplot(df, aes(x = reorder(id, utilisation),
                   y = utilisation * 100, fill = scenario)) +
      geom_bar(stat = "identity", position = "dodge", alpha = 0.85) +
      coord_flip() +
      scale_fill_manual(values = scenario_palette[seq_along(sc)]) +
      labs(x = NULL, y = "Utilisation (%)", fill = "Scenario") +
      theme_minimal(base_size = 13)
  })
}
